...
const howMuchPencil = (str) => {
  ...
  // Solution code here...
  // str.forEach((element) => {
  //   result.push(str.slice(element));
  // }); (this worked online but not in vscode here)
...
const listFoods = (recipe) => {
  ...
  for ( let i=0 ; i < recipe.ingredients.length; i++){
    let ind = recipe.ingredients[i].indexOf(' ',6);
    result.push(recipe.ingredients[i].slice(ind + 1));
  }
  return result;
};
